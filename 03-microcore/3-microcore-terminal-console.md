...
3. Rendre persistant le clavier

```shell
echo "kmaps.tcz" | sudo tee -a /mnt/sda1/tce/onboot.lst
```

4. Appliquer la kmap au démarrage : `sudo vi /opt/bootlocal.sh` avec le contenu suivant :
...
4. Chargement de l'IPv6 au boot

```shell
echo "ipv6-3.16.6-tinycore.tcz" | sudo tee -a /mnt/sda1/tce/onboot.lst
```

5. Chargement du module au démarrage : `sudo vi /opt/bootlocal.sh` avec le contenu suivant :
...
Tada ! Le troisième projet est terminé.
